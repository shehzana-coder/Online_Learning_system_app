...
// Update the _submitBooking method with working navigation
  // Update the _submitBooking method to fetch student details
  Future<void> _submitBooking() async {
    try {
      ...
      final studentEmail = studentData['email'] ?? user.email ?? 'No email';
      final studentPhone = studentData['phone'] ?? 'No phone';
      final studentId = studentData['studentId'] ?? 'No ID';
      final studentDepartment = studentData['department'] ?? 'No department';
      final studentSemester = studentData['semester'] ?? 'No semester';
      ...
      // Create booking document with all student details
...
// Update the _buildAddDetailsButton to call _submitBooking when confirmed
  Widget _buildAddDetailsButton(BuildContext context) {
    return SizedBox(
      width: double.infinity,
      height: 56,
      child: ElevatedButton(
        onPressed: _isConfirmed ? _submitBooking : null,
        style: ElevatedButton.styleFrom(
