...
// Assuming TutorInfoScreen is the previous screen

class Seemyschedule extends StatefulWidget {
  ...
  @override
  State<Seemyschedule> createState() => _SessionSchedulingScreenState();
}

class _SessionSchedulingScreenState extends State<Seemyschedule> {
  ...
  // Availability and tutor details
  ...
  Future<void> _fetchAvailabilityAndDetails() async {
    try {
      ...
      // Fetch from verified teachers collection
      ...
      if (verifiedDoc.exists) {
        ...
        DocumentSnapshot notVerifiedDoc =
            await _firestore.collection('teachers').doc(widget.tutorId).get();
        if (notVerifiedDoc.exists) {
          _tutorDetails = notVerifiedDoc.data() as Map<String, dynamic>;
          _tutorAvailability =
              _convertAvailabilityFormat(_tutorDetails?['availability'] ??
                  {
                    'timezone': 'Not specified',
          ...
          _errorMessage =
              'Teacher not found in verified or unverified teachers';
        }
      }
